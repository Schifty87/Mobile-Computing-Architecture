...
class Trout {
	...
	public ArrayList<map> down(map M, ArrayList<point> P, int Xcoord, int Ycoord, String name){
		String A = M.getA();
		String B = M.getB();
		int X1 = 0;
		int X2 = 0;
	...
	public String alreadyMade(int X, int Y, ArrayList<point> P){
		for(int i=0;i<P.size();i++){
			if(P.get(i).getX()==X && P.get(i).getY()==Y){
				return P.get(i).getPoint();
			}
		}
		return null;
	}
